...
# $group
#                     diff      lwr      upr     p adj
# offshore-inshore 1.767237 -11.8048 15.33928 0.7896446
...
tukey_site
# only marginal significiant comparison
# $group
#                     diff      lwr      upr     p adj
# PopaIsland-BastimentosN    38.5622455  -0.1223854 77.24688 0.0510353
...
# Response: Distances
#           Df Sum Sq Mean Sq F value Pr(>F)
# Groups     2  370.9  185.47  0.3916 0.6837 

tukey_time <- TukeyHSD(disp_time, which = "group", ordered = FALSE, conf.level = 0.95)
tukey_time
# $group
# diff       lwr      upr     p adj
# Mid-Early  -9.379640 -50.01030 31.25102 0.8174948
# Late-Early  3.910651 -28.05738 35.87868 0.9443542
# Late-Mid   13.290291 -26.36117 52.94176 0.6587743
...
maxTempJune_forLM <- c(30.43,29.51,29.51,29.70,29.51,29.70,29.22,
                       29.70,NA,29.92,29.22,NA,29.92,29.22,NA,29.92,
                       NA,NA,NA,NA,30.43,NA,NA,30.43)

plot(maxTempJune_forLM~firstAxisScores)
lm_temp_PC1 <- lm(maxTempJune_forLM~firstAxisScores)
summary(lm_temp_PC1)
anova_temp_PC1 <- anova(lm_temp_PC1)
abline(coef(lm_temp_PC1), col="red")
legend("bottomright", paste("p =", round(anova_temp_PC1$`Pr(>F)`[1],2), sep=" "), bty="n")
